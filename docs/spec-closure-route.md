...
| Task checkboxes | 588 / 605 checked |
...
| --- | --- | --- |
| 001 Time-Series Evidence Substrate | Broad substrate is substantially implemented through historical blocks; `tasks.md` is 239 / 244 checked and `blocks/` has 93 files. Not closed: T214-T217 and T226 remain open. Active demo evidence is in `fall-out-bug/sdp-trace-demo-jvm-gsd`: T211 is accepted with `sh_test` semantic behavior evidence over the compiled app, T212 is accepted as superseded by the v2 packet/bundle CI artifact contract, and T213 is accepted with demo PR #25 merged to `main` plus downloaded-artifact replay over demo CI run `25724386343`. T226 current-route replay observes `minimax/MiniMax-M2.5`, source commit, output digest, and normalized digest, but current OpenCode/GSD lacks `/gsd-plan-phase`; validation remains `not_assessed` for tool/phase/mutation/test. | Split into closed historical blocks vs live blockers. Block 25 still needs negative-state, sanitized-report, role-review, and final verification evidence for T214-T217. T226 requires a current GSD route that emits delivery-loop evidence; keep unavailable dimensions explicit, not green. |
...
Current task ledger state is 588 / 605 checked, leaving 17 open task boxes.
...
| External demo / first-run work | 001 T214-T217, T226 | Real remaining work outside this repo's current local artifacts. Needs demo-repository execution, retained evidence, and review. |
...
3. **External demo surface**: resolve 001 T214-T217 against the active
...
| Block 25 demo evidence | 001 T214-T217 | Active demo repo is `fall-out-bug/sdp-trace-demo-jvm-gsd`; T211/T212/T213 are accepted. Negative-state, sanitized-report, role-review, and final verification evidence still need mapping. | Continue T214-T217 against the active demo evidence; keep missing evidence open. |
