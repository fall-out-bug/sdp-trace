...
| Task checkboxes | 592 / 605 checked |
...
| --- | --- | --- |
| 001 Time-Series Evidence Substrate | Broad substrate is substantially implemented through historical blocks; `tasks.md` is 243 / 244 checked and `blocks/` has 93 files. Not closed: T226 remains open. Active demo evidence is in `fall-out-bug/sdp-trace-demo-jvm-gsd`: T211 is accepted with `sh_test` semantic behavior evidence over the compiled app, T212 is accepted as superseded by the v2 packet/bundle CI artifact contract, T213 is accepted with demo PR #25 merged to `main` plus downloaded-artifact replay over demo CI run `25724386343`, T214 is accepted with demo PR #26 merged to `main` plus GitHub `build-and-test` run `26447797437`, T215 is accepted with `docs/reviews/block25-jvm-gsd-demo-sanitized-report.md`, T216 is accepted with active-demo role reviews plus fixed redaction-scan evidence, and T217 is accepted with fresh local verification plus PR #64 `verify` and `pr-review-evidence-only` success at head `ea00be499abe9a211f3fa639be6124863afad36c`. T226 GSD-Redux replay observes `minimax/MiniMax-M2.5`, source commit, output digest, normalized digest, and tool events; validation remains `not_assessed` for phase/mutation/test. | Split into closed historical blocks vs live blockers. T226 requires the GSD-Redux route to emit required delivery-loop evidence or a successor split for unavailable dimensions; keep unavailable dimensions explicit, not green. |
...
Current task ledger state is 592 / 605 checked, leaving 13 open task boxes.
...
| External demo / first-run work | 001 T226 | Real remaining work outside this repo's current local artifacts. Needs current GSD-Redux route execution, retained phase/mutation/test evidence, and review. |
...
3. **External demo surface**: resolve 001 T226 only after a current
   OpenCode/GSD-Redux route emits first-run delivery-loop evidence or the
   missing normalized dimensions are explicitly split into a successor
   task/spec.
...
| T226 first-run OpenCode/GSD observation | 001 T226 | GSD-Redux replacement observes setup/model/source/digest/tool facts, but phase/mutation/test remain `not_assessed`. | Close normalizer evidence for GSD-Redux phase/mutation/test, or split unavailable dimensions from observer capability. |

The remaining risk is no longer hidden implementation work. It is authority:
merge approval, maintainer approval/waiver/split decisions, and external
repository or harness availability.
