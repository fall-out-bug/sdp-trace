...
| External demo / first-run work | 001 T226 | Real remaining work outside this repo's current local artifacts. Needs current-route execution, retained evidence, and review. |
...
3. **External demo surface**: resolve 001 T226 only after a current
   OpenCode/GSD route emits first-run delivery-loop evidence or the missing
   route is explicitly split into a successor task/spec.
