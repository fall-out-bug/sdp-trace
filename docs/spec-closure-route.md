...
| --- | --- | --- |
| 001 Time-Series Evidence Substrate | Broad substrate is substantially implemented through historical blocks; `tasks.md` is 238 / 244 checked and `blocks/` has 93 files. Not closed: T213-T217 and T226 remain open. Active demo evidence is in `fall-out-bug/sdp-trace-demo-jvm-gsd`: T211 is accepted with `sh_test` semantic behavior evidence over the compiled app, and T212 is accepted as superseded by the v2 packet/bundle CI artifact contract. T213's old artifact-index shape is obsolete; demo PR #25 implements v2 downloaded-artifact verification and passes local Bazel tests, but GitHub has not emitted a PR workflow run, so downloaded CI artifacts are unavailable for replay. T226 current-route replay observes `minimax/MiniMax-M2.5`, source commit, output digest, and normalized digest, but current OpenCode/GSD lacks `/gsd-plan-phase`; validation remains `not_assessed` for tool/phase/mutation/test. | Split into closed historical blocks vs live blockers. Block 25 still needs v2 downloaded-artifact digest verification, negative-state, sanitized-report, role-review, and final verification evidence for T213-T217. T226 requires a current GSD route that emits delivery-loop evidence; keep unavailable dimensions explicit, not green. |
...
| Block 25 demo evidence | 001 T213-T217 | Active demo repo is `fall-out-bug/sdp-trace-demo-jvm-gsd`; T211/T212 are accepted; T213 old artifact-index shape is obsolete and demo PR #25 implements the v2 verifier, but no #25 PR workflow run exists yet for downloaded-artifact replay. Negative-state, sanitized-report, role-review, and final verification evidence still need mapping after T213. | Resolve #25 workflow event, replay downloaded CI artifacts, then continue T214-T217; keep missing evidence open. |
