...
| D007 | 001 T213-T217 | Complete remaining Block 25 v2 artifact verification, negative-state, sanitized-report, role-review, and final verification evidence against `fall-out-bug/sdp-trace-demo-jvm-gsd`. | T211 is accepted with active demo repo evidence and `kt_jvm_test` wording recorded as drift; T212 is accepted as superseded by the v2 packet/bundle CI artifact contract; T213's old artifact-index shape is accepted as obsolete. Demo PR #25 implements v2 artifact verification and passes local Bazel tests, but GitHub has not emitted a PR workflow run, so downloaded-artifact replay remains open. | V2 bundle artifact verification over downloaded CI artifacts, negative-state replay, sanitized report, role reviews, and local/PR-level verification. | `not_assessed` |
