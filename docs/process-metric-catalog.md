...
| `manifest_digest_match_state` | enum | Whether contract manifest artifact digests matched the checkout. Values: `matched`, `mismatch`, `not_assessed`. | Run or read manifest verification evidence. | `sdp-trace release-proof --manifest <manifest> --out <file>`, contract release verification. | Use `not_assessed` if manifest or verification output is missing. | Low. |
| `release_attestation_state` | enum | Current release proof state. Values: `schema_valid`, `digest_verified`, `locally_attested`, `externally_attested`, `production_release_verified`, `not_assessed`. | Read release-proof or external trust verification output. | `sdp-trace release-proof --manifest <manifest> --out <file>`, release verification record, external trust evidence. | Use `not_assessed` for any proof state not backed by committed or externally inspectable evidence. | Medium when external signer identity is private. |
...
| `observed_interaction_event_count` | count | Count of retained interaction events by event type, source, and task scope. | Count validated interaction events in an interaction trace. | `interaction-trace.schema.json`, delivery trace envelope refs. | Use `not_assessed` if interaction sources are absent, incomplete, or not comparable. | Medium because interaction metadata can describe private work patterns. |
| `observed_post_assignment_correction_count` | count | Count of corrective feedback, boundary violation, and evidence correction events after task assignment. | Count validated interaction events after the first `task_assignment` event. | Interaction trace events and delivery trace envelope refs. | Use `not_assessed` when task assignment is absent or source ordering cannot be verified. | Medium; must not be used as a quality score without downstream context. |
| `observed_plan_rejection_count` | count | Count of observed plan rejection events before approved execution. | Count `plan_rejected` interaction events in task scope. | Interaction trace events. | Use `not_assessed` when plan phase events are absent or transcript completeness is not assessed. | Low to medium. |
| `observed_clarification_turn_count` | count | Count of clarification request and answer events in the task interaction trace. | Count `clarification_request` and `clarification_answer` events. | Interaction trace events. | Use `not_assessed` when interaction source is absent or partiality prevents counting. | Low to medium. |
| `unreferenced_interaction_event_count` | count | Count of observed interaction events not linked to downstream plan, run, evidence, review, PR, or mutation refs. | Count interaction events with unreferenced state or empty reference refs. | Interaction trace and delivery trace envelope. | Use `not_assessed` if downstream artifact refs are unavailable. | Low. |
| `interaction_source_completeness_state` | enum | Completeness state of the interaction source. Values: `complete`, `partial`, `not_assessed`, `cannot_verify`. | Read source completeness fields from the interaction trace. | Interaction trace source metadata. | Use `not_assessed` if source completeness metadata is absent. | Low. |
