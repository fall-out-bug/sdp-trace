...
| `checkpoint verify` | Verify a signed checkpoint against a run and optional policy. | `sdp-trace checkpoint verify --run .sdp-trace-runs/run-1 --checkpoint checkpoint.json` | Replays checkpoint evidence; does not upgrade trust scope beyond the checkpoint's original authority. Pass `--policy` to enforce a signer authorization policy. |
