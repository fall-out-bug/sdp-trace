...
| `observe collect` | Collect declared harness observation output after the normal harness command ran. | `go run ./cmd/sdp-trace observe collect --profile session-profile.json --run session-run` | Normalizes profile-declared event output into an observed harness run without asking the operator to author events. Missing source output is `cannot_verify`. |
