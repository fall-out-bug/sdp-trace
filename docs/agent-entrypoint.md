...
| `packet build-pr` | Build live PR packet artifacts from GitHub Actions context or offline GitHub fixtures without checked-in packet edits. | `sdp-trace packet build-pr --source github-actions --out packet-artifacts` | Authoritative live-demo packet path for this slice. Writes `bundle.json`, `change-evidence-packet.md`, and `build-pr-result.json`; `PC-VERIFICATION` must bind to workflow/artifact evidence or remains `cannot_verify`. |
