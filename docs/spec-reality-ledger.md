...
|---|---|---|---|---|---|---|---|
| **001** | sdp-trace Time-Series Evidence Substrate | `draft` (blocked) | Draft | 238 / 244 | T213–T217, T226 | `go test` PASS, `go vet` PASS, `doccheck` PASS, `schemadoc` PASS. **CRAP PASS**. **MI baseline PASS**. **Absolute MI gap remains assessed**. Block 21 export replay passed; PR #64 `pr-review-evidence-only` workflow evidence passed for Block 32 closure refresh. Block 25 Socratic review ledger exists. Active demo evidence is `fall-out-bug/sdp-trace-demo-jvm-gsd`: T211 is accepted with `kt_jvm_library`, Bazel CI, and semantic `sh_test` endpoint evidence over the compiled app; T212 is accepted as superseded by the v2 packet/bundle CI artifact contract; T213's old artifact-index shape is obsolete, demo PR #25 implements v2 downloaded-artifact verification and passes local Bazel tests, but no #25 PR workflow run exists yet for downloaded CI artifact replay. T226 live recheck against OpenCode 1.15.10 collects with current route `minimax/MiniMax-M2.5`, source commit, output digest, and normalized digest, but the current OpenCode/GSD environment does not expose `/gsd-plan-phase`; validation remains `not_assessed` for tool/phase/mutation/test. | **Partial** — many blocks implemented and tested locally, but T213-T217 and T226 remain open and spec status is draft. |
