...
|---|---|---|---|---|---|---|---|
| **001** | sdp-trace Time-Series Evidence Substrate | `draft` (blocked) | Draft | 236 / 244 | T211–T217, T226 | `go test` PASS, `go vet` PASS, `doccheck` PASS, `schemadoc` PASS. **CRAP PASS**. **MI baseline PASS**. **Absolute MI gap remains assessed**. Block 21 export replay passed; PR #64 `pr-review-evidence-only` workflow evidence passed for Block 32 closure refresh. Block 25 Socratic review ledger exists, and PR #19 merged with `verify` PASS, but the external demo repository does not resolve via current GitHub access. | **Partial** — many blocks implemented and tested locally, but open demo/first-run tasks remain and spec status is draft. |
...
| **007** | GitHub OSS Demo Packet | `draft` (old) | Spec reviewed / approval and demo implementation pending | 7 / 22 | T008-T022 | `go test` PASS, `doccheck` PASS. Historical review ledger now includes 2026-05-26 focused split re-review. Local demo repository path is absent in this environment. | **Partial** — spec package is reviewed, but approval and current demo-repository packetization evidence are missing. |
...
- **Demo / first-run specs**: 001 Block 25/T226 and 007 Phase 2 remain real
  ...
  cleanup.
- **Spec 007 T008-T022**: Demo-track approval and demo-repository evidence
  remain open; the local demo repo is not present in this environment.
