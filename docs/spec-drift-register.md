...
| Spec 008 invisible flight recorder | `not_assessed` | Spec status and command examples now match the implemented local `observe session` / `packet build-pr` surface, but PR/final-head CI evidence is still pending per the post-implementation ledger. | Do not claim PR-backed closure until the PR and live GitHub checks are observed. Current docs remain authoritative for command usage. |
...
| Quality gates | `assessed_gap` | CRAP `< 5`, cyclomatic `<= 10`, and cognitive `<= 10` now pass locally for production Go under `cmd`, `internal`, and `tools` and are wired into CI. Function-MI ratchet passes for `cmd`/`internal`; file-MI ratchet passes for `cmd`/`internal`/`tools`. Absolute function/file MI `> 70` still fails broadly on historical code and tool helpers and is not claimed as pass. | Keep ratcheting historical MI baselines down before claiming absolute MI pass. Final-head GitHub CI remains external evidence and must be observed before CI-backed closure. |
