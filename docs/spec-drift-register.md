...
| Quality gates | `pass_local_requires_live_ci_check` | CRAP `< 5`, cyclomatic `<= 10`, cognitive `<= 10`, absolute function MI `> 70`, and absolute file MI `> 70` now pass locally for Go under `cmd`, `internal`, and `tools`; the local MI replay uses threshold `70.1` to avoid treating rounded `70.0` rows as closure. CI still enforces the MI baseline ratchets rather than the absolute MI commands, because the baseline policy protects future PRs from introducing new exceptions. Checked-in prose is not CI authority. | Observe final-head GitHub CI outside this source file after each push. Do not use the MI metric alone as Clean Code proof; review must still account for file organization and trust-boundary readability. |
