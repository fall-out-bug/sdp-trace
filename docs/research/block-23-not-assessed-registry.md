...
| B23-NA-07 | Block 06 Beads mirror source-bound proof | `not_assessed` | `.beads/issues.jsonl` and `bd ready` are process/backlog evidence, not manifest subjects and not shipped `sdp-trace` commands | `.beads/issues.jsonl`, `bd ready` | accepted process reclassification; no product-trust claim |
