...
- [x] T209 [US3] Run Socratic spec review for Block 25 and record every valid
  critical or major finding in
  `blocks/25-compiled-ci-demo-pilot-review-ledger.md`.
- [x] T210 [US3] Close every valid Block 25 Socratic spec finding before
  implementation starts.
  Evidence: `blocks/25-compiled-ci-demo-pilot-review-ledger.md` records
  Socratic spec review planes and accepted/fixed findings. Current closure
  route verified the ledger is present in this source tree. Demo-repository
  implementation and artifact replay remain open below.
- [x] T211 [US3] Update the demo repository with a pinned `kt_jvm_library` and
  `kt_jvm_test` compiled behavior target and semantic tests; keep source/scope
  checks secondary.
  Evidence: `fall-out-bug/sdp-trace-demo-jvm-gsd` is the active demo repository.
  Current evidence shows `app/BUILD.bazel` uses `kt_jvm_library`, CI runs
  `bazel build //...` and `bazel test //...`, PRs #16-#23 report
  `build-and-test` SUCCESS, and `app/smoke_test.sh` exercises semantic endpoint
  behavior for `/health`, `/info`, `/version`, `/ready`, and `/live`. The
  original literal `kt_jvm_test` wording is accepted as drift for this demo
  track because the retained behavior test is a Bazel `sh_test` over the
  compiled app.
- [x] T212 [US4] Update demo CI so `.sdp-trace-report/`, `.sdp-trace-runs/`,
  and `.sdp-trace-tools/` are generated by CI, not committed as authority.
  Evidence: accepted as superseded by the active `sdp-trace-demo-jvm-gsd`
  packetization contract. Current CI uploads `.sdp-trace/packets/` and
  `.sdp-trace/bundles/` artifacts, and the closure route keeps checked-in
