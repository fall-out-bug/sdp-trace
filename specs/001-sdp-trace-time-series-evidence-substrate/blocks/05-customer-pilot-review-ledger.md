...
- `spec.md`
- `tasks.md`
- Existing pilot target artifacts under `archive/research/`, `docs/harness-compatibility-matrix.md`, `docs/model-compatibility.md`, `docs/jvm-bazel-guide.md`, and `examples/jvm-bazel/`
...
| F009 | major | `sdp-trace-cdn.22.18` | Existing run-card template owns native pass/warn/fail verdicts. | Accepted; implementation fixed. | `archive/research/run-card-template.md` now uses evidence states, reason codes, artifact references, and optional external-verdict input. |
| F010 | major | `sdp-trace-cdn.22.16` | Harness matrix hides evidence-state boundaries and combines `gsd`/`gsd2`. | Accepted; implementation fixed. | `docs/harness-compatibility-matrix.md` now splits `gsd`/`gsd2` and records evidence state, reason code, artifact reference, external verdict reference, gap reason, and next evidence. |
| F011 | major | `sdp-trace-cdn.22.15` | Model matrix omits OpenCode+MiniMax/Kimi/GLM rows. | Accepted; implementation fixed. | `docs/model-compatibility.md` now includes explicit OpenCode+MiniMax, OpenCode+Kimi, and OpenCode+GLM rows as `not_assessed`. |
...
| F019 | minor/P3 | `sdp-trace-cdn.22.9` | Package naming blurred Block 05 design and customer handoff outline. | Accepted; spec layer revised. | Block file title now says "Block 05 Design"; `archive/research/customer-pilot-evidence-package.md` remains the T037 handoff outline. |
