...
| CRAP `< 5` | Fresh coverage plus cyclomatic output replayed through `tools/crapcheck -strict-less` for `cmd`, `internal`, and `tools`. | `pass_local`: strict CRAP replay exited 0 locally with fresh `/tmp/sdp-trace-export-contract-split-cover.out`, `/tmp/sdp-trace-export-contract-split-cover-func.txt`, and `/tmp/sdp-trace-export-contract-split-gocyclo.txt`. |
...
| Maintainability Index `> 70` | Absolute function/file MI check exits 0 without relying on exception baselines. | `assessed_gap`: absolute file MI still fails with 27 file rows and absolute function MI still fails with 1288 stderr rows. Current local ratchets pass, including the new trace/export/contract split files, but repository-wide absolute MI is not achieved. |
...
| `CRAP < 5` | `go test -count=1 ./... -coverprofile=/tmp/sdp-trace-export-contract-split-cover.out`; `go tool cover -func=/tmp/sdp-trace-export-contract-split-cover.out > /tmp/sdp-trace-export-contract-split-cover-func.txt`; `go run ./tools/qualitycheck -gocyclo cmd internal tools > /tmp/sdp-trace-export-contract-split-gocyclo.txt`; `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-export-contract-split-cover-func.txt -gocyclo /tmp/sdp-trace-export-contract-split-gocyclo.txt -threshold 5 -strict-less`. | `pass_local` |
...
| `Maintainability Index > 70` | `go run ./tools/qualitycheck -mi-under 70 cmd internal tools` exits 1 with 27 stderr rows; `go run ./tools/qualitycheck -function-mi-under 70 cmd internal tools` exits 1 with 1288 stderr rows. Ratchet commands with checked-in baselines exit 0, and the new trace/export/contract split files pass absolute file/function MI. | `assessed_gap` |
...
1. Absolute Maintainability Index `> 70` is not achieved. Latest local replay
   still fails with 27 file-level stderr rows and 1288 function-level stderr
...
- `go test -count=1 ./... -coverprofile=/tmp/sdp-trace-export-contract-split-cover.out`
