...
| Maintainability Index `> 70` | Absolute function/file MI check exits 0 without relying on exception baselines, using threshold `70.1` so rounded `70.0` rows do not satisfy the stricter-than-70 claim. | `fail_local`: replay on 2026-05-17 showed absolute file MI and absolute function MI still fail under `cmd`, `internal`, and `tools`. CI uses baseline ratchets, not absolute MI closure. |
...
| Documentation completeness | README/docs/schema/example docs cover changed behavior and trust scope without external-trust overclaims. | `pass_with_advisory_findings`: `go run ./tools/doccheck` passes and docs now mark absolute MI `> 70` as failed locally rather than closed. Schema README generation/validation remains an advisory follow-up. |
...
| `Maintainability Index > 70` | `go run ./tools/qualitycheck -fail-only -mi-under 70.1 cmd internal tools`; `go run ./tools/qualitycheck -fail-only -function-mi-under 70.1 cmd internal tools`; both exited 1 on 2026-05-17 with current absolute MI failures. | `fail_local` |
...
- `go run ./tools/qualitycheck -fail-only -mi-under 70.1 cmd internal tools` (`fail_local`: current absolute file MI failures remain)
- `go run ./tools/qualitycheck -fail-only -function-mi-under 70.1 cmd internal tools` (`fail_local`: current absolute function MI failures remain)
...
All listed local commands exited 0 except the two explicit absolute MI replay
commands, which exited 1 on 2026-05-17. Absolute file MI and absolute function
MI without baselines are not closed for Go under `cmd`, `internal`, and `tools`
at threshold `70.1`. PR #43 `verify` must be live-queried after each push and
bound to the exact head outside this file, because checked-in source text does
not by itself prove CI-backed closure for future heads.
