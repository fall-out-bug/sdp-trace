...
| Maintainability Index `> 70` | Absolute function/file MI check exits 0 without relying on exception baselines. | `assessed_gap`: absolute file MI still fails with 29 file rows and absolute function MI still fails with 1301 stderr rows. Current local ratchets pass, including the new trace split files, but repository-wide absolute MI is not achieved. |
...
| `Maintainability Index > 70` | `go run ./tools/qualitycheck -mi-under 70 cmd internal tools` exits 1 with 29 stderr rows; `go run ./tools/qualitycheck -function-mi-under 70 cmd internal tools` exits 1 with 1301 stderr rows. Ratchet commands with checked-in baselines exit 0, and the new trace split files pass absolute file/function MI. | `assessed_gap` |
...
1. Absolute Maintainability Index `> 70` is not achieved. Latest local replay
   still fails with 29 file-level stderr rows and 1301 function-level stderr
