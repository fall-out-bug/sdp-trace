...
| `go test ./... -coverprofile=/tmp/sdp-trace-prreview-profile-full.out` | pass | Total coverage: 74.7%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-prreview-profile-full-func.txt -gocyclo /tmp/sdp-trace-prreview-profile-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 373 functions exceed strict CRAP threshold 5; `internal/prreview.validateProfile` and its extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `internal/prreview.citationResolvable` | 13 | 27.3% | 77.94 | assessed_gap |
...
3. `internal/prreview.citationResolvable`
4. `internal/managed.capabilityCondition`
5. `internal/harnessobs.safeParentDir`
6. `internal/harnessobs.Validate`
7. `internal/forensic.prewriteCondition`
