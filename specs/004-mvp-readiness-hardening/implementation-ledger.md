...
| `go test ./... -coverprofile=/tmp/sdp-trace-demo-gate-full.out` | pass | Total coverage: 74.4%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-demo-gate-full-func.txt -gocyclo /tmp/sdp-trace-demo-gate-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 377 functions exceed strict CRAP threshold 5; `internal/demo.EvaluateGate` and its extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `cmd/sdp-trace.runValidateFixtures` | 14 | 60.6% | 25.99 | assessed_gap |
...
3. `cmd/sdp-trace.runValidateFixtures`
4. `cmd/sdp-trace.runPRReviewCheck`
5. `internal/telemetry.unsafeValue`
6. `internal/prreview.citationResolvable`
7. `internal/prreview.validateProfile`
