...
| Coverage hardening | pass_partial | MVP-critical zero-coverage packages `contract`, `export`, and `policy` now have focused tests; `cmd/sdp-trace` (71.7%), `authority` (89.0%), `adaptercapture` (84.3%), `checkpoint` (77.2%), `ciartifact` (92.9%), `contract` (89.1%), `demo` (79.9%), `forensic` (90.4%), `harnessobs` (80.5%), `interaction` (69.7%), `managed` (88.9%), `posture` (89.4%), `prreview` (79.6%), `query` (92.0%), `repoobserver` (71.7%), `trace` (62.2%), `verifier` (83.4%), and `witness` (78.2%) were improved. |
...
| `go test ./... -coverprofile=/tmp/sdp-trace-batch10-full.out` | pass | Total coverage: 78.4%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-batch10-full-func.txt -gocyclo /tmp/sdp-trace-batch10-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 329 functions exceed strict CRAP threshold 5; `internal/prreview.RunReview`, `internal/posture.verifyDigestManifest`, `internal/demo.overrideRequestsFromEvents`, `internal/checkpoint.applyPolicy`, and their extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `internal/posture.validateMovementRow` | 10 | 66.7% | 13.69 | assessed_gap |
...
1. `cmd/sdp-trace.run`
2. `internal/posture.validateMovementRow`
3. `internal/posture.validateExportCollections`
4. `internal/posture.validateExportRows`
5. `internal/posture.ingestRepository`
6. `internal/forensic.validateRawReference`
7. `internal/demo.evaluateRequiredRuns`
