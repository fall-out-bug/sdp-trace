...
| `go test ./... -coverprofile=/tmp/sdp-trace-witness-match-full.out` | pass | Total coverage: 73.9%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-witness-match-full-func.txt -gocyclo /tmp/sdp-trace-witness-match-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 383 functions exceed strict CRAP threshold 5; `cmd/sdp-trace.witnessMatchesProtectedInput` is now CRAP 3.00, but the repo-wide strict target remains open. |
