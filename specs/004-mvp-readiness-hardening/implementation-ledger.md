...
| `go test ./... -coverprofile=/tmp/sdp-trace-batch7-full.out` | pass | Total coverage: 77.0%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-batch7-full-func.txt -gocyclo /tmp/sdp-trace-batch7-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 345 functions exceed strict CRAP threshold 5; `cmd/sdp-trace.runQueryPack`, `cmd/sdp-trace.runProtectedGate`, `cmd/sdp-trace.runAssess`, and their extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `internal/witness.FetchGitHubOIDCToken` | 10 | 0.0% | 110.00 | assessed_gap |
...
1. `cmd/sdp-trace.run`
2. `internal/witness.FetchGitHubOIDCToken`
3. `internal/witness.loadCustomerPublicKey`
4. `internal/trace.ValidateRunDirectory`
5. `internal/repoobserver.updateGitignore`
6. `internal/repoobserver.writeInstallFiles`
7. `internal/query.mapSourceState`
