...
| `go test ./... -coverprofile=/tmp/sdp-trace-harnessobs-shell-full.out` | pass | Total coverage: 72.9%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-harnessobs-shell-full-func.txt -gocyclo /tmp/sdp-trace-harnessobs-shell-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 391 functions exceed strict CRAP threshold 5; `internal/harnessobs.shellFields` was removed from the over-15 offender list, and harnessobs now has no production cyclomatic offender above 15. |
