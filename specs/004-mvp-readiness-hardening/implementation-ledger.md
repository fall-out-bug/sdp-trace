...
| Coverage hardening | pass_partial | MVP-critical zero-coverage packages `contract`, `export`, and `policy` now have focused tests; `cmd/sdp-trace` (71.7%), `authority` (89.0%), `adaptercapture` (84.3%), `ciartifact` (92.9%), `contract` (89.1%), `demo` (74.4%), `forensic` (90.4%), `harnessobs` (80.5%), `interaction` (69.7%), `managed` (88.9%), `posture` (89.1%), `prreview` (78.0%), `query` (92.0%), `repoobserver` (71.7%), `trace` (62.2%), `verifier` (83.4%), and `witness` (78.2%) were improved. |
...
| `go test ./... -coverprofile=/tmp/sdp-trace-batch9-full.out` | pass | Total coverage: 77.7%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-batch9-full-func.txt -gocyclo /tmp/sdp-trace-batch9-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 336 functions exceed strict CRAP threshold 5; `internal/witness.loadCustomerPublicKey`, `internal/prreview.safeID`, `internal/posture.unsafeOutput`, `internal/harnessobs.findStringByKeyIn`, `internal/harnessobs.findNumberByKeyIn`, and their extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `internal/prreview.RunReview` | 10 | 73.9% | 11.78 | assessed_gap |
| `internal/posture.verifyDigestManifest` | 10 | 77.3% | 11.17 | assessed_gap |
...
1. `cmd/sdp-trace.run`
2. `internal/prreview.RunReview`
3. `internal/posture.verifyDigestManifest`
4. `internal/posture.validateMovementRow`
5. `internal/posture.validateExportCollections`
6. `internal/demo.overrideRequestsFromEvents`
7. `internal/checkpoint.applyPolicy`
