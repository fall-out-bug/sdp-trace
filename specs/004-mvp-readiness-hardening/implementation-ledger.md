...
| Coverage hardening | pass_partial | MVP-critical zero-coverage packages `contract`, `export`, and `policy` now have focused tests; `demo` (73.8%), `forensic` (84.0%), `harnessobs` (71.3%), `interaction` (67.3%), `posture` (87.8%), `prreview` (73.9%), `trace`, `verifier`, and `witness` (71.2%) were improved. |
...
| `go test ./... -coverprofile=/tmp/sdp-trace-posture-build-full.out` | pass | Total coverage: 73.0%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-posture-build-full-func.txt -gocyclo /tmp/sdp-trace-posture-build-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 392 functions exceed strict CRAP threshold 5; `internal/posture.Build` was removed from the over-15 offender list, but extracted helpers add below-15 CRAP entries until covered or further decomposed. |
...
1. `cmd/sdp-trace.run`
2. `internal/adaptercapture.runBindingCondition`
3. `cmd/sdp-trace.(*flagSet).parse`
4. `internal/recorder.Run`
5. `internal/ciartifact.evaluateFamily`
6. `internal/adaptercapture.overclaimCondition`
