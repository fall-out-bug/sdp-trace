...
| `go test ./... -coverprofile=/tmp/sdp-trace-telemetry-full.out` | pass | Total coverage: 74.6%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-telemetry-full-func.txt -gocyclo /tmp/sdp-trace-telemetry-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 374 functions exceed strict CRAP threshold 5; `internal/telemetry.unsafeValue` and its extracted helper are now below threshold, but the repo-wide strict target remains open. |
...
4. `internal/prreview.validateProfile`
5. `internal/managed.capabilityCondition`
6. `internal/harnessobs.safeParentDir`
7. `internal/harnessobs.Validate`
