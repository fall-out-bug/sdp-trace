...
| Coverage hardening | pass_partial | MVP-critical zero-coverage packages `contract`, `export`, and `policy` now have focused tests; `forensic` (84.0%), `harnessobs`, `interaction` (67.3%), `prreview` (73.9%), `trace`, `verifier`, and `witness` (71.2%) were improved. |
...
| `go test ./... -coverprofile=/tmp/sdp-trace-prreview-runner-cover.out` | pass | Total coverage: 72.0%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-prreview-runner-cover-func.txt -gocyclo /tmp/sdp-trace-prreview-runner-gocyclo.txt -threshold 5` | fail_assessed_gap | 389 functions exceed strict CRAP threshold 5; `internal/prreview.runRole` and `internal/prreview.BuildPacket` were removed from >15 complexity findings, but extracted helpers still exceed CRAP 5 at current coverage. |
...
3. `internal/demo.witnessBindingState`
4. `internal/harnessobs.findUnsafeRawEventAt`
5. `internal/harnessobs.findUnsafeAt`
6. `internal/harnessobs.validateEvent`
