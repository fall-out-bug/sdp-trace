...
| `go test ./... -coverprofile=/tmp/sdp-trace-prreview-citation-full.out` | pass | Total coverage: 74.9%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-prreview-citation-full-func.txt -gocyclo /tmp/sdp-trace-prreview-citation-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 372 functions exceed strict CRAP threshold 5; `internal/prreview.citationResolvable` and its extracted helpers are now below threshold, but the repo-wide strict target remains open. |
...
| `internal/managed.capabilityCondition` | 13 | 87.5% | 13.33 | assessed_gap |
...
3. `internal/managed.capabilityCondition`
4. `internal/harnessobs.safeParentDir`
5. `internal/harnessobs.Validate`
6. `internal/forensic.prewriteCondition`
7. `cmd/sdp-trace.runWitness`
