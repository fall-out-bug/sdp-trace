...
| `go test ./... -coverprofile=/tmp/sdp-trace-adaptercapture-binding-full.out` | pass | Total coverage: 73.1%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-adaptercapture-binding-full-func.txt -gocyclo /tmp/sdp-trace-adaptercapture-binding-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 391 functions exceed strict CRAP threshold 5; `internal/adaptercapture.runBindingCondition` was removed from the over-15 offender list, but the repo-wide strict target remains open. |
...
5. `internal/adaptercapture.overclaimCondition`
6. `cmd/sdp-trace.runGateExplain`
