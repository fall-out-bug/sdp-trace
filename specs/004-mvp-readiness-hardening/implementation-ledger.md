...
| `go test ./... -coverprofile=/tmp/sdp-trace-gate-explain-full.out` | pass | Total coverage: 74.2%. |
...
| `go run ./tools/crapcheck -cover-func /tmp/sdp-trace-gate-explain-full-func.txt -gocyclo /tmp/sdp-trace-gate-explain-full-gocyclo.txt -threshold 5` | fail_assessed_gap | 380 functions exceed strict CRAP threshold 5; `cmd/sdp-trace.runGateExplain` is now below threshold, but the repo-wide strict target remains open. |
...
| `internal/harnessobs.Observe` | 14 | 72.7% | 17.99 | assessed_gap |
...
3. `internal/harnessobs.Observe`
4. `internal/forensic.policyCondition`
5. `internal/demo.EvaluateGate`
6. `cmd/sdp-trace.runValidateFixtures`
7. `cmd/sdp-trace.runPRReviewCheck`
