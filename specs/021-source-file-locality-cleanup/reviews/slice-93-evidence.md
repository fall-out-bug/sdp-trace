...
Scope: `internal/prreview` coverage-state, model-identity support, and summary
rendering shards `prreview_069` through `prreview_078`.

## Source Shape

- `find internal/prreview -maxdepth 1 -type f | sed 's#^#/#' | rg '/prreview_0(6[9]|7[0-8])_[^/]+\.go$' || true`
  - verified: no output; numbered Slice 93 files are gone.
- `git diff --name-only | rg '^internal/prreview/prreview_(0(7[9]|8[0-9]|9[0-9])|1[0-9][0-9])_' || true`
  - verified: no output; excluded `prreview_079+` files were not moved or
    edited in Slice 93.

## Verification

- Focused exact-count guard and regression tests:
  `tests='TestSynthesizeAndValidateCoverageSatisfied|TestValidateCannotVerifyUnexplainedModelMismatch|TestValidateCoverageStatesForNoReviewersUnresolvedAndStaleDigest|TestValidationAndSummaryRedactUnsafeMarkerClasses|TestPrreviewCoverageModelAndSummaryRenderingContracts'; listed=$(go test ./internal/prreview -list "$tests" | rg "^($tests)$"); test "$(printf '%s\n' "$listed" | sed '/^$/d' | wc -l | tr -d ' ')" = 5 && go test ./internal/prreview -run "$tests" -count=1 -v`
  - verified: pass.
