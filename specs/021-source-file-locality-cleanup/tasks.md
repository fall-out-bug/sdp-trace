...
## Active Slice 121 Tasks

- [x] T021-8390 Confirm Slice 121 is bounded to `internal/harnessobs`
  validation dimension artifact shape files: `dimension_type.go` and
  `validation_type.go`.
- [x] T021-8391 Confirm Slice 121 is behavior-preserving: no changes to
  dimension composition, validation state/reason selection, validation loading,
  summary rendering behavior, schemas, examples, dependencies, package
  boundary, dependency direction, CRAP < 5, MI > 70, or MI baselines are
  planned.
- [x] T021-8392 Run three independent plan/task reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record the plan review in
  `specs/021-source-file-locality-cleanup/reviews/slice-121-plan-review.md`.
- [x] T021-8400 Consolidate `dimension_type.go` into `validation_type.go`
  without creating replacement micro-shards. Record source-shape evidence that
  the dimension type shard is gone and staged boundary evidence that excluded
  harness observation behavior files and public surfaces are untouched.
- [x] T021-8410 Run `gofmt` on changed Go files.
- [x] T021-8420 Run focused harness observation dimension artifact verification
  with a `go test -list` exact-count guard expecting exactly 2 tests:
  `TestValidateWritesOutPathWhenPasses` and
  `TestSummarizeDoesNotPrintUnsafeValues`, then run them with
  `go test ./internal/harnessobs -run <guard> -count=1 -v`. The focused test
  set must preserve the `Dimension` JSON fields/tags inside persisted
  validation artifacts and preserve summary rendering from `Validation`
  dimensions. If current coverage lacks exact dimension field/tag assertions,
  extend one of the two guarded tests before implementation review; do not add
  a third focused test name unless this task is re-reviewed.
- [x] T021-8430 Run repository verification: `go test ./...`, `go vet ./...`,
  `golangci-lint run` when available or record `not_assessed`/`cannot_verify`
  with reason, `go run ./tools/doccheck`, `go run ./tools/hygienecheck`, `jq
  empty schema/*.json`, and `git diff --check`.
- [x] T021-8440 Run CRAP and MI quality gates without changing MI baselines.
- [x] T021-8450 Run three independent implementation reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record Slice 121 evidence in
  `specs/021-source-file-locality-cleanup/reviews/slice-121-evidence.md`,
  including harness, agent id, model/provider if available, date, prompt class,
  timeout, retries, fallback, result, and any unavailable external/provider
  lanes as `not_assessed`.

## Active Slice 122 Tasks

- [x] T021-8460 Confirm Slice 122 is bounded to `internal/harnessobs`
  observation event artifact shape files: `event_type.go` and `run_type.go`.
- [x] T021-8461 Confirm Slice 122 is behavior-preserving: no changes to event
  decoding, identity/ref/content validation, event scanning, event writing
  behavior, run loading, normalized event generation, schemas, examples,
  dependencies, package boundary, dependency direction, CRAP < 5, MI > 70, or
  MI baselines are planned.
- [x] T021-8462 Run three independent plan/task reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record the plan review in
  `specs/021-source-file-locality-cleanup/reviews/slice-122-plan-review.md`.
- [x] T021-8470 Consolidate `event_type.go` into `run_type.go` without
  creating replacement micro-shards. Record source-shape evidence that the
  event type shard is gone and staged boundary evidence that excluded harness
  observation behavior files and public surfaces are untouched.
- [x] T021-8480 Run `gofmt` on changed Go files.
- [x] T021-8490 Run focused harness observation event artifact verification
  with a `go test -list` exact-count guard expecting exactly 2 tests:
  `TestWriteNormalizedEventsWritesJSONL` and
  `TestCollectSessionWritesObservedRun`, then run them with
  `go test ./internal/harnessobs -run <guard> -count=1 -v`. The focused test
  set must preserve the `Event` JSON fields/tags inside persisted JSONL events
  and preserve observation run/event reference artifact wiring. If current
  coverage lacks exact event field/tag assertions, extend one of the two
  guarded tests before implementation review; do not add a third focused test
  name unless this task is re-reviewed.
- [x] T021-8500 Run repository verification: `go test ./...`, `go vet ./...`,
  `golangci-lint run` when available or record `not_assessed`/`cannot_verify`
  with reason, `go run ./tools/doccheck`, `go run ./tools/hygienecheck`, `jq
  empty schema/*.json`, and `git diff --check`.
- [x] T021-8510 Run CRAP and MI quality gates without changing MI baselines.
- [x] T021-8520 Run three independent implementation reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record Slice 122 evidence in
  `specs/021-source-file-locality-cleanup/reviews/slice-122-evidence.md`,
  including harness, agent id, model/provider if available, date, prompt class,
  timeout, retries, fallback, result, and any unavailable external/provider
  lanes as `not_assessed`.
...
## Active Slice 124 Tasks

- [x] T021-8600 Confirm Slice 124 is bounded to `internal/harnessobs`
  observation profile degradation rule artifact shape files: `rule_type.go`
  and `profile_type.go`.
- [x] T021-8601 Confirm Slice 124 is behavior-preserving: no changes to
  degradation rule validation, profile loading, observation/validation
  behavior, schemas, examples, dependencies, package boundary, dependency
  direction, CRAP < 5, MI > 70, or MI baselines are planned.
- [x] T021-8602 Run three independent plan/task reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record the plan review in
  `specs/021-source-file-locality-cleanup/reviews/slice-124-plan-review.md`.
- [x] T021-8610 Consolidate `rule_type.go` into `profile_type.go` without
  creating replacement micro-shards. Record source-shape evidence that the rule
  type shard is gone and staged boundary evidence that excluded harness
  observation behavior files and public surfaces are untouched.
- [x] T021-8620 Run `gofmt` on changed Go files.
- [x] T021-8630 Run focused harness observation profile rule verification with
  a `go test -list` exact-count guard expecting exactly 2 tests:
  `TestLoadProfileValidatesProfile` and `TestObserveValidateCompleteHarnessExport`,
  then run them with `go test ./internal/harnessobs -run <guard> -count=1 -v`.
  The focused test set must preserve the `Rule` JSON fields/tags inside the
  `Profile` artifact shape and keep profile validation plus complete
  observation/validation behavior covered. If current coverage lacks exact
  `Rule` field/tag assertions, extend one of the two guarded tests before
  implementation review. The assertion must prove zero-value `Rule{}` emits
  both `state` and `reason_code`, and populated rules emit both exact field
  names. Do not add a third focused test name unless this task is re-reviewed.
- [x] T021-8640 Run repository verification: `go test ./...`, `go vet ./...`,
  `golangci-lint run` when available or record `not_assessed`/`cannot_verify`
  with reason, `go run ./tools/doccheck`, `go run ./tools/hygienecheck`, `jq
  empty schema/*.json`, and `git diff --check`.
- [x] T021-8650 Run CRAP and MI quality gates without changing MI baselines.
- [x] T021-8660 Run three independent implementation reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record Slice 124 evidence in
  `specs/021-source-file-locality-cleanup/reviews/slice-124-evidence.md`,
  including harness, agent id, model/provider if available, date, prompt class,
  timeout, retries, fallback, result, and any unavailable external/provider
  lanes as `not_assessed`.

## Active Slice 125 Tasks

- [x] T021-8670 Confirm Slice 125 is bounded to `internal/harnessobs`
  unavailable event-field artifact shape files: `unavailable_field_type.go`
  and `run_type.go`.
- [x] T021-8671 Confirm Slice 125 is behavior-preserving: no changes to
  unavailable field validation, event content validation, event decoding, event
  writing behavior, run loading, schemas, examples, dependencies, package
  boundary, dependency direction, CRAP < 5, MI > 70, or MI baselines are
  planned.
- [x] T021-8672 Run three independent plan/task reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record the plan review in
  `specs/021-source-file-locality-cleanup/reviews/slice-125-plan-review.md`.
- [x] T021-8680 Consolidate `unavailable_field_type.go` into `run_type.go`
  without creating replacement micro-shards. Record source-shape evidence that
  the unavailable-field type shard is gone and staged boundary evidence that
  excluded harness observation behavior files and public surfaces are
  untouched.
- [x] T021-8690 Run `gofmt` on changed Go files.
- [x] T021-8700 Run focused harness observation unavailable-field artifact
  verification with a `go test -list` exact-count guard expecting exactly 2
  tests: `TestWriteNormalizedEventsWritesJSONL` and
  `TestValidateEventRejectsInvalidFields`, then run them with
  `go test ./internal/harnessobs -run <guard> -count=1 -v`. The focused test
  set must preserve the `UnavailableField` JSON fields/tags inside persisted
  `Event` JSONL and keep unavailable field validation covered. Before
  implementation review, extend one of the two guarded tests with raw JSON
  assertions proving zero-value `UnavailableField{}` emits `field`, `state`,
  and `reason_code`, and populated unavailable fields emit all three exact field
  names. Do not add a third focused test name unless this task is re-reviewed.
- [x] T021-8710 Run repository verification: `go test ./...`, `go vet ./...`,
  `golangci-lint run` when available or record `not_assessed`/`cannot_verify`
  with reason, `go run ./tools/doccheck`, `go run ./tools/hygienecheck`, `jq
  empty schema/*.json`, and `git diff --check`.
- [x] T021-8720 Run CRAP and MI quality gates without changing MI baselines.
- [x] T021-8730 Run three independent implementation reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record Slice 125 evidence in
  `specs/021-source-file-locality-cleanup/reviews/slice-125-evidence.md`,
  including harness, agent id, model/provider if available, date, prompt class,
  timeout, retries, fallback, result, and any unavailable external/provider
  lanes as `not_assessed`.
...
## Active Slice 129 Tasks

- [x] T021-8950 Confirm Slice 129 is bounded to residual product `_type.go`
  cleanup for `tools/qualitycheck/options_type.go`.
- [x] T021-8951 Confirm Slice 129 is behavior-preserving: no flag
  registration changes, no default path changes, no CLI exit behavior changes,
  no threshold semantic changes, no MI baseline behavior changes, no report
  rendering changes, no dependencies/package boundary changes, no CRAP < 5 or
  MI > 70 regression, and no MI baseline changes are planned.
- [x] T021-8952 Run three independent plan/task reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record the plan review in
  `specs/021-source-file-locality-cleanup/reviews/slice-129-plan-review.md`.
- [x] T021-8960 Consolidate `options_type.go` into `options.go` without
  changing the unexported `options` declaration. Record source-shape evidence
  that `options_type.go` is gone and no replacement `_type.go` shard remains
  in product paths.
- [x] T021-8970 Run `gofmt` on changed Go files.
- [x] T021-8980 Run focused qualitycheck CLI option verification with a
  `go test -list` exact-count guard expecting exactly 4 tests after adding the
  missing focused test `TestParseOptionsPopulatesAllOptionFields`:
  `TestParseOptionsPopulatesAllOptionFields`,
  `TestRunFailOnlySuppressesPassingMetricRows`, and
  `TestFunctionMaintainabilityBaselineAllowsHistoricalLowMI`, and
  `TestFileMaintainabilityBaselineAllowsHistoricalLowMI`, then run them with
  `go test ./tools/qualitycheck -run <guard> -count=1 -v`. The focused test set
  must prove `parseOptions`/`optionsFromValues` field population and
  default/zero-value behavior for every `options` field (`cycloOver`,
  `cognitiveOver`, `miUnder`, `fileMIBaseline`, `writeFileMIBaseline`,
  `functionMIUnder`, `functionMIBaseline`, `writeFunctionMIBaseline`,
  `gocyclo`, `failOnly`, and `err`), default analysis paths, plus downstream
  CLI fail-only/report and both file/function MI baseline option behavior after
  consolidation.
- [x] T021-8990 Run repository verification: `go test ./...`, `go vet ./...`,
  `golangci-lint run` when available or record `not_assessed`/`cannot_verify`
  with reason, `go run ./tools/doccheck`, `go run ./tools/hygienecheck`, `jq
  empty schema/*.json`, and `git diff --check`.
- [x] T021-9000 Run CRAP and MI quality gates without changing MI baselines.
- [x] T021-9010 Run three independent implementation reviewer lanes, fix every
  finding, repeat affected lanes until each reviewer returns exactly `LGTM`,
  and record Slice 129 evidence in
  `specs/021-source-file-locality-cleanup/reviews/slice-129-evidence.md`,
  including harness, agent id, model/provider if available, date, prompt class,
  timeout, retries, fallback, result, and any unavailable external/provider
  lanes as `not_assessed`.
