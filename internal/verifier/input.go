...
func verifiedRunInput(runDir string) (trace.RunManifest, []trace.Event, trace.VerifierResult, *trace.IntegrityAudit, error, bool) {
	...
	events, result, audit, ok := verifiedEvents(runDir, manifest)
	return manifest, events, result, audit, nil, ok
}
...
func verifiedEvents(runDir string, manifest trace.RunManifest) ([]trace.Event, trace.VerifierResult, *trace.IntegrityAudit, bool) {
	...
	if err != nil {
		...
		return nil, cannotVerifyResult(runDir, err.Error()), audit(manifest.RunID, "event_load_failed", err.Error(), "run_dir", runDir), false
	}
	if len(events) == 0 {
		...
		return nil, result, audit(manifest.RunID, "empty_events", "run contains no events", "run_dir", runDir), false
	}
	return events, trace.VerifierResult{}, nil, true
}
